...
在本实验室中，你将使用在本模块中学习到的信息来监视和优化之前迁移的数据库。你将运行一个示例应用程序，该应用程序使数据库承受读取操作繁重的工作负载，并使用 Azure 中可用的指标监视结果。你将使用查询存储来检查查询的性能。然后，你将配置只读副本，并将某些客户端的读取处理卸载到该副本。然后，你将检查此项更改如何影响性能。
...
1. 使用 Azure 指标来监视性能。
1. 使用查询存储检查查询的性能。
1. 卸载某些用户的读取操作以使用只读副本。
1. 再次监视性能并评估结果。

## 应用场景

你是 AdventureWorks 组织的数据库开发人员。AdventureWorks 十多年来一直直接向终端消费者和经销商销售自行车和自行车零件。他们的系统将信息存储在你先前已迁移到 Azure Database for PostgreSQL 的数据库中。

执行迁移之后，你要确保系统运行良好。你决定使用可用的 Azure 工具来监视服务器。为了降低争用和延迟引起响应时间变慢的可能性，你决定实现读取复制。你需要监视结果系统，并将结果与单一服务器体系结构进行比较。
...
1. 如果以前未执行此操作，请在 Cloud Shell 中，克隆包含脚本和示例数据库的存储库。

    ```bash
    git clone https://github.com/MicrosoftLearning/DP-070ZH-Migrate-Open-Source-Workloads-to-Azure ~/workshop
    ```

1. 转至 *workshop/migration_samples/setup/postgresql* 文件夹。

    ```bash
    cd ~/workshop/migration_samples/setup/postgresql
    ```

1. 请运行以下命令。将 *[nnn]* 替换为一个数字，确保该脚本创建的 Azure Database for PostgreSQL 服务器具有唯一名称。在指定资源组为脚本的第二个参数的 Azure 资源组中创建服务器。如果该资源组尚不存在，则进行创建。或者，将资源组位置指定为第三个参数。如果省略参数，则此位置默认为“westus”：

    ```bash
    bash copy_adventureworks.sh adventureworks[nnn] [resource_group_name] [location]
    ```

1. 当提示你输入“*awadmin*”密码时，输入“**Pa55w.rdDemo**”。
1. 当提示你输入“*azureuser*”密码时，输入“**Pa55w.rd**”。
1. 等待脚本完成后再继续。此操作将需要 5 到 10 分钟，完成时显示消息“**设置完成**”。
...
1. 运行一个示例应用程序，该应用程序模拟查询数据库的多个用户。
1. 查看指标。

### 任务 1：为你的 Azure Database for PostgreSQL 服务配置 Azure 指标

1. 以“**azureuser**”用户的身份登录在教室环境运行的 **LON-DEV-01** 虚拟机，登录密码为“**Pa55w.rd**”。
1. 使用浏览器，登录至 Azure 门户。
1. 在 Azure 门户中，转到 Azure Database for PostgreSQL 服务页面。
1. 在“**监视**”下，选择“**指标**”。
1. 在图表页面上，添加以下指标：

    | 属性  | 值  |
    |---|---|
    | 作用域 | adventureworks[nnn] |
...
1. 选择“**添加指标**”，并添加以下指标：
    ...
    最后三个指标显示了测试应用对资源的消耗情况。

1. 将图表的时间范围设置为**最近 30 分钟**。
1. 选择“**固定到仪表板**”，然后选择“**固定**”。

### 任务 2：运行一个示例应用程序，该应用程序模拟查询数据库的多个用户

1. 在 Azure 门户中，在 Azure Database for PostgreSQL 服务器页面上的“**设置**”下，选择“**连接字符串**”。将 ADO.NET 连接字符串复制到剪贴板。
1. 切换到 Azure Cloud shell。如果以前未执行此操作，请克隆包含脚本和示例数据库的存储库。

    ```bash
    git clone https://github.com/MicrosoftLearning/DP-070ZH-Migrate-Open-Source-Workloads-to-Azure ~/workshop
    ```

1. 移动到 *~/workshop/migration_samples/code/postgresql/AdventureWorksSoakTest* 文件夹。
...
1. 将**数据库**的值替换为 **azureadventureworks**，并将 **ConectionString0** 替换为剪贴板中的连接字符串。将“**用户 ID**”更改为“**azureuser@adventureworks[nnn]**”，然后将“**密码**”设置为“**Pa55w.rd**”。完成的文件看起来类似于以下示例：
    ...
    > [注意！]
    > 暂时忽略 **ConnectionString1** 和 **ConnectionString2** 设置。你将稍后在实验室中更新这些项目。

1. 保存更改并关闭编辑器。
1. 在 Cloud Shell 提示符下，运行以下命令来构建和运行应用程序：

    ```bash
    dotnet run
    ```

    应用启动时，它将生成许多线程，每个线程模拟一个用户。线程执行循环，运行一系列查询。你将看到如下所示的消息开始出现：
...
1. 选择图表以在“**指标**”窗格中将其打开。
1. 允许应用运行几分钟（时间越长越好）。久而久之，图表中的指标应类似于下图所示的模式：
...
1. 在 Azure 门户中，在 Azure Database for PostgreSQL 服务器页面上的“**设置**”下，选择“**服务器参数**”。
1. 在“**服务器参数**”页面上，将以下参数设置为下表中指定的值。

    | 参数  | 值  |
    |---|---|
    | pg_qs.max_query_text_length | 6000 |
    | pg_qs.query_capture_mode | 所有 |
    | pg_qs.replace_parameter_placeholders | 打开 |
    | pg_qs.retention_period_in_days | 7 |
    | pg_qs.track_utility | 打开 |
    | pg_stat_statements.track | 所有 |
    | pgms_wait_sampling.history_period | 100 |
    | pgms_wait_sampling.query_capture_mode | 所有 |
...
1. 在 Azure Database for PostgreSQL 服务器页面上，在“**智能性能**”下选择“**Query Performance Insight**”。
1. 在“**Query Performance Insight**”页面上的“**长期查询**”选项卡上，将“**查询数量**”设置为 10，将“**选择依据**”设为“**平均值**”，然后将“**时间段**”设为“**过去 6 小时**”。
1. 在图表上方，多次选择“**放大**”（带有“+”号的放大镜图标），以放大最新数据。
...
1. 选择“**等待统计**”选项卡。
1. 将“**时间段**”设置为“**过去 6 小时**”，将“**分组依据**”设为“**事件**”，并将“**组数上限**”设为 **5**。
    ...
    > [注意！]
    > 对数据库的读写操作由“**IO**”事件（而不是“**客户端**”事件）指示。该示例应用程序不会引起任何 IO 等待，因为它需要的所有数据在第一次读取后都会缓存在内存中。如果指标表明内存容量不足，你可能会看到 IO 等待事件开始发生。
...
1. 在 Azure 门户中，在 Azure Database for PostgreSQL 服务器页面上的“**设置**”下，选择“**复制**”。
1. 在“**复制**”页面上，选择“**添加副本**”。
1. 在“**PostgreSQL 服务器**”页面的“**服务器名称**”框中，键入“**adventureworks[nnn]-replica1**”，然后选择“**确定**”。
...
1. 选择 **adventureworks[nnn]-replica1** 副本的名称。你将转到该副本的 Azure Database for PostgreSQL 页面。
1. 在“**设置**”下，选择“**连接安全性**”。
1. 在“**连接安全性**”页面，将“**允许访问 Azure 服务**”设为“**开**”，然后选择“**保存**”。此设置使你使用 Cloud Shell 运行的应用程序能够访问服务器。
1. 保存设置后，重复前面的步骤并允许 Azure 服务访问“**adventureworks[nnn]-replica2**”副本。

### 任务 3：重新启动每台服务器

> [注意！]
> 配置复制不需要重启服务器。该任务的目的是清除每个服务器上的内存和任何无关的连接，以便在再次运行该应用程序时收集的指标已得到*清理*。

1. 转到 **adventureworks[nnn]** 服务器页面。
1. 在“**概述**”页面上，选择“**重启**”。
1. 在“**重启服务器**”对话框中，选择“**是**”。
1. 等待服务器重启后再继续。
1. 按照相同的步骤，重启 **adventureworks[nnn]-replica1** 和 **adventureworks[nnn]-replica2** 服务器。
...
1. 为“**ConnectionString1**”和“**ConnectionString2**”设置添加连接字符串。这些值应与“**ConnectionString0**”的相同，但带有文字“**adventureworks[nnn]**”被替换为“**服务器**”和“**用户 ID**”元素中的“**adventureworks[nnn]-replica1**”和“**adventureworks[nnn]-replica2**”。
1. 将“**NumReplicas**”设置为 **3**。
...
1. 返回到 Cloud Shell，然后按 Enter 停止应用程序。
